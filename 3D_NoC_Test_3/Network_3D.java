...
public class Network_3D {
	...
	public void createMeshNetwork() {
		...
		IConstants_3D.NUMBER_OF_IP_NODE = IConstants_3D.NUMBER_OF_SWITCH
				* noOfAdjNode;
		...
		for (i = 0; i < IConstants_3D.NUMBER_OF_SWITCH; i++) {
			...
			// add IP Nodes to the Mesh Switch_3D
			...
			for (int k = 0; k < noOfAdjNode; k++) {
				...
				if (IConstants_3D.ASYNCHRONOUS)
					nd = new Node_3D(address + k, meshSwitch, k,
							IConstants_3D.CURRENT_VC_COUNT, factor);
				else
					nd = new Node_3D(address + k, meshSwitch, k,
	...
	private void setAdjacentMeshSwitch() {
		...
		// int numSwitch = IConstants_3D.NUMBER_OF_IP_NODE /
		...
		for (i = 0; i < numSwitch; i++) {
			...
			if (row > 0) {
				...
				meshSwitch.setAdjacentSwitch((MeshSwitch_3D) switchList
						.get(((row - 1) * IConstants_3D.MESH_COL) + col),
			...
			if (col > 0) {
				// left
				meshSwitch.setAdjacentSwitch((MeshSwitch_3D) switchList
						.get((row * IConstants_3D.MESH_COL) + (col - 1)),
	 ...
	 * 
	 * @see Network_3D#setAdjacentFatSwitch()
	 * @see Network_3D#setAdjacentExFatSwitch()
	 */
	private int getStartOfLevel(int level) {
		int i, count = 0;
		for (i = 1; i < level; i++)
			count += IConstants_3D.NUMBER_OF_IP_NODE / (1 << (i + 1));
		return count;
	}
	...
	public void setInitalEvents() {
		...
		// track no of link active in each Switch_3D. For statistical purpose
		for (i = 0; i < switchList.size(); i++) {
			nocSwitch = (Switch_3D) switchList.get(i);
